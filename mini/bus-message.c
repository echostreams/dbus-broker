...
_public_ int sd_bus_message_close_container(sd_bus_message* m) {
    ...
    if (c->enclosing == SD_BUS_TYPE_ARRAY)
        ...
        ((c->enclosing == SD_BUS_TYPE_STRUCT || c->enclosing == SD_BUS_TYPE_DICT_ENTRY))
...
int bus_body_part_map(struct bus_body_part* part) {
    ...
    if (part->memfd < 0 && part->is_zero && part->size < 8) {
        static const uint8_t zeroes[7] = { 0, 0, 0, 0, 0, 0, 0 };
...
static int container_next_item(sd_bus_message* m, struct bus_container* c, size_t* rindex) {
    ...
    if (c->enclosing == SD_BUS_TYPE_ARRAY) {
        ...
        ((c->enclosing == 0 || c->enclosing == SD_BUS_TYPE_STRUCT || c->enclosing == SD_BUS_TYPE_DICT_ENTRY))
    {
...
_public_ int sd_bus_message_read_basic(sd_bus_message* m, char type, void* p) {
    ...
    if (BUS_MESSAGE_IS_GVARIANT(m)) {
        ...
        if //(IN_SET(type, SD_BUS_TYPE_STRING, SD_BUS_TYPE_OBJECT_PATH, SD_BUS_TYPE_SIGNATURE)) 
            ((type == SD_BUS_TYPE_STRING || type == SD_BUS_TYPE_OBJECT_PATH || type == SD_BUS_TYPE_SIGNATURE))
        {
            bool ok;
        ...
        if //(IN_SET(type, SD_BUS_TYPE_STRING, SD_BUS_TYPE_OBJECT_PATH)) 
            ((type == SD_BUS_TYPE_STRING || type == SD_BUS_TYPE_OBJECT_PATH))
        {
            uint32_t l;
            bool ok;
...
_public_ int sd_bus_message_skip(sd_bus_message* m, const char* types) {
    ...
    if (!types) {
        ...
        const char* _t = c->signature + c->index;
        //p = (char*)memdupa_suffix0(_t, strnlen(_t, MAX(1, e - path)));
        void* _q_;
        size_t _l_ = strnlen(_t, l);
...
static int message_skip_fields(
    ...
    for (;;) {
        ...
        if (t == SD_BUS_TYPE_STRING) {
            ...
            ((t == SD_BUS_TYPE_STRUCT || t == SD_BUS_TYPE_DICT_ENTRY))
        {
...
_public_ int sd_bus_message_copy(sd_bus_message* m, sd_bus_message* source, int all) {
    ...
    do {
        ...
        if //(IN_SET(type, SD_BUS_TYPE_OBJECT_PATH, SD_BUS_TYPE_SIGNATURE, SD_BUS_TYPE_STRING))
            ((type == SD_BUS_TYPE_OBJECT_PATH || type == SD_BUS_TYPE_SIGNATURE || type == SD_BUS_TYPE_STRING))
