...
static void base_remove_entry(HashmapBase* h, unsigned idx) {
    ...
    for (right = next_idx(h, left); ; right = next_idx(h, right)) {
        ...
        if //(IN_SET(raw_dib, 0, DIB_RAW_FREE))
            ((raw_dib == 0 || raw_dib == DIB_RAW_FREE))
            break;
...
static struct HashmapBase* hashmap_base_new(const struct hash_ops* hash_ops, enum HashmapType type  HASHMAP_DEBUG_PARAMS) {
    ...
    LIST_PREPEND(debug_list, struct hashmap_debug_info, hashmap_debug_list, &h->debug);
...
#if ENABLE_DEBUG_HASHMAP
    assert_se(pthread_mutex_lock(&hashmap_debug_list_mutex) == 0);
    LIST_REMOVE(debug_list, struct hashmap_debug_info, hashmap_debug_list, &h->debug);
    assert_se(pthread_mutex_unlock(&hashmap_debug_list_mutex) == 0);
#endif
...
void _hashmap_clear(HashmapBase* h, free_func_t default_free_key, free_func_t default_free_value) {
    ...
    free_key = h->hash_ops->free_key ? h->hash_ops->free_key : default_free_key;
    free_value = h->hash_ops->free_value ? h->hash_ops->free_value : default_free_value;
...
static bool hashmap_put_robin_hood(HashmapBase* h, unsigned idx,
    ...
    for (distance = 0; ; distance++) {
        ...
        if //(IN_SET(raw_dib, DIB_RAW_FREE, DIB_RAW_REHASH)) 
            ((raw_dib == DIB_RAW_FREE || raw_dib == DIB_RAW_REHASH))
        {
