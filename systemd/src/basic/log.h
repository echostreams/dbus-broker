...
inline int __log_full_errno_zerook(int level, int error, const char* format, ...)
{     
    va_list ap;
    int _level = (level), _e = (error);
    va_start(ap, format);
...
inline int __log_full_errno(int level, int error, const char* format, ...)
{
    va_list ap;
    va_start(ap, format);
    int _error = (error);                                   
...
#define log_full(level, fmt, ...)                                      \
        ({                                                             \
                if (BUILD_MODE_DEVELOPER)                              \
                        assert(!strstr(fmt, "%m"));                    \
                (void)log_internal(level, 0, __FILE__, __LINE__, __func__, fmt, ##__VA_ARGS__); \
        })
*/
#define log_full(level, fmt, ...)                                      \
        do {                                                             \
