...
static struct node *bus_node_allocate(sd_bus *bus, const char *path) {
        ...
        if (streq(path, "/"))
                ...
                const char* _t = path;
                //p = (char*)memdupa_suffix0(_t, strnlen(_t, MAX(1, e - path)));
                void* _q_;
                size_t _l_ = strnlen(_t, MAX(1, e - path));
                //_q_ = alloca_safe(_l_ + 1);     
                size_t _nn_ = _l_ + 1;
                assert(_nn_ <= ALLOCA_MAX);
                _q_ = alloca(_nn_ == 0 ? 1 : _nn_);
                ((uint8_t*)_q_)[_l_] = 0;
                p = (char*)memcpy_safe(_q_, path, _l_);
        ...
        if (parent)
                LIST_PREPEND(siblings, struct node, parent->child, n);
...
void bus_node_gc(sd_bus *b, struct node *n) {
        ...
        if (n->parent)
                LIST_REMOVE(siblings, struct node, n->parent->child, n);
...
static int bus_add_object(
        ...
        LIST_PREPEND(callbacks, struct node_callback, n->callbacks, &s->node_callback);
...
static int add_object_vtable_internal(
        ...
        LIST_INSERT_AFTER(vtables, struct node_vtable, n->vtables, existing, &s->node_vtable);
        bus->nodes_modified = true;
...
_public_ int sd_bus_add_node_enumerator(
        ...
        LIST_PREPEND(enumerators, struct node_enumerator, n->enumerators, &s->node_enumerator);
        bus->nodes_modified = true;
...
_public_ int sd_bus_emit_properties_changed(
        ...
        strv_from_stdarg_alloca(names, name);
...
_public_ int sd_bus_emit_interfaces_added(sd_bus *bus, const char *path, const char *interface, ...) {
        ...
        strv_from_stdarg_alloca(interfaces, interface);
...
_public_ int sd_bus_add_object_manager(sd_bus *bus, sd_bus_slot **slot, const char *path) {
        ...
        LIST_PREPEND(object_managers, struct node_object_manager, n->object_managers, &s->node_object_manager);
        bus->nodes_modified = true;
