...
static void bus_track_add_to_queue(sd_bus_track *track) {
        ...
        LIST_PREPEND(queue, sd_bus_track, track->bus->track_queue, track);
        track->in_queue = true;
}
...
static void bus_track_remove_from_queue(sd_bus_track *track) {
        ...
        LIST_REMOVE(queue, sd_bus_track, track->bus->track_queue, track);
...
_public_ int sd_bus_track_new(
        ...
        LIST_PREPEND(tracks, sd_bus_track, bus->tracks, t);
        t->in_list = true;
...
static sd_bus_track *track_free(sd_bus_track *track) {
        ...
        if (track->in_list)
                LIST_REMOVE(tracks, sd_bus_track, track->bus->tracks, track);
