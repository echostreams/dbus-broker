...
sd_bus_slot *bus_slot_allocate(
        ...
        LIST_PREPEND(slots, sd_bus_slot, bus->slots, slot);
...
void bus_slot_disconnect(sd_bus_slot *slot, bool unref) {
        ...
        case BUS_FILTER_CALLBACK:
                ...
                LIST_REMOVE(callbacks, struct filter_callback, slot->bus->filter_callbacks, &slot->filter_callback);
                break;
        ...
        case BUS_MATCH_CALLBACK:
                ...
                slot->match_callback.match_string = NULL;
                break;

        case BUS_NODE_CALLBACK:

                if (slot->node_callback.node) {
                        LIST_REMOVE(callbacks, struct node_callback, slot->node_callback.node->callbacks, &slot->node_callback);
        ...
        case BUS_NODE_ENUMERATOR:
                ...
                if (slot->node_enumerator.node) {
                        LIST_REMOVE(enumerators, struct node_enumerator, slot->node_enumerator.node->enumerators, &slot->node_enumerator);
                        slot->bus->nodes_modified = true;
        ...
        case BUS_NODE_OBJECT_MANAGER:
                ...
                if (slot->node_object_manager.node) {
                        LIST_REMOVE(object_managers, struct node_object_manager, slot->node_object_manager.node->object_managers, &slot->node_object_manager);
                        slot->bus->nodes_modified = true;
        ...
        case BUS_NODE_VTABLE:
                ...
                slot->node_vtable.interface = NULL;

                if (slot->node_vtable.node) {
                        LIST_REMOVE(vtables, struct node_vtable, slot->node_vtable.node->vtables, &slot->node_vtable);
                        slot->bus->nodes_modified = true;
        ...
        slot->bus = NULL;
        LIST_REMOVE(slots, sd_bus_slot, bus->slots, slot);
